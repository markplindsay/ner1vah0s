...
export default {
  handleNameChunkDrag: draggedNameChunk => (dispatch, getState) => {
    const adjustment = getState().getIn(['app', 'adjustment'])
    const chunkBcr = draggedNameChunk.el.getBoundingClientRect()
    const nameBcr = getState().getIn(['app', 'nameEl']).getBoundingClientRect()
    dispatch({
  ...
  setWindowSize: (h, w) => {
    ...
    return {
      ...
      payload: Immutable.Map({ adjustment, xOffset }),
    }
  },
}
